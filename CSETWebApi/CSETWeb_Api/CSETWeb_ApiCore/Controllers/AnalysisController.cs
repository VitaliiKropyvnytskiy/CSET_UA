    ...
    public class AnalysisController : ControllerBase
    {
        ...
        public IActionResult GetFeedback()
        {
            try
            {
                ...
                string FeedbackSalutations = "Шановний  " + (FaaMail ? "FAA" : "CSET") + " Адміністратор стандартів:";
                string FeedbackDescription = "До кожного із запитань були надані наступні коментарі: ";
                string FeedbackWarning = " *** Обов'язково *** Збережіть цей ідентифікатор питання ***";


                FeedbackResult.FeedbackHeader = "Надіслати відгук до DHS";
                if (FaaMail) FeedbackResult.FeedbackHeader += " та FAA";
                ...
                FeedbackResult.FeedbackBody = "Будь ласка, напишіть на адресу: <br/><br/>";
                ...
                foreach (FeedbackQuestion q in feedbackQuestions)
                {
                    ...
                    FeedbackResult.FeedbackBody += "Відгуки користувачів: <br/>" + q.Feedback + "<br/><br/>";
                    FeedbackResult.FeedbackBody += q.QuestionText + "<br/><br/>";
                    FeedbackResult.FeedbackBody += FeedbackWarning + "<br/>";
                    FeedbackResult.FeedbackBody += "Питання #" + " " + q.Mode + ":" + q.QuestionID + ". <br/><br/><br/>";
                }

                FeedbackResult.FeedbackEmailSubject = "CSET відгуки про питання";
                ...
                foreach (FeedbackQuestion q in feedbackQuestions)
                {
                    ...
                    FeedbackResult.FeedbackEmailBody += "Відгуки користувачів: %0D%0A" + q.Feedback + "%0D%0A";
                    FeedbackResult.FeedbackEmailBody += q.QuestionText + "%0D%0A%0D%0A";
                    FeedbackResult.FeedbackEmailBody += FeedbackWarning + "%0D%0A";
                    FeedbackResult.FeedbackEmailBody += "Питання #" + " " + q.Mode + ":" + q.QuestionID + ". %0D%0A%0D%0A%0D%0A";
                }

                if (feedbackQuestions.Count() == 0)
                {
                    FeedbackResult.FeedbackBody = "Немає відгуків щодо будь-яких запитань у цьому оцінюванні";
                }
        ...
        public IActionResult GetDashboard()
        {
            ...
            if (results.Count >= 2)
            {
                ...
                var g = compliance.Where(x => x.Item1 == "Overall")
                                  .Select(x => new Tuple<string, double>("Загалом", x.Item2))
                                  .FirstOrDefault();
                if (g != null)
                {
                ...
                g = compliance.Where(x => x.Item1 == "Standards")
                              .Select(x => new Tuple<string, double>("Стандарти", x.Item2))
                              .FirstOrDefault();
                if (g != null)
                {
                    complianceOrdered.Add(g);
                ...
                g = compliance.Where(x => x.Item1 == "Components")
                              .Select(x => new Tuple<string, double>("Компоненти", x.Item2))
                              .FirstOrDefault();
                if (g != null)
                {
                    complianceOrdered.Add(g);
        ...
        public IActionResult GetOverallRankedCategories()
        {
            ...
            if (results.Count >= 1)
            {
                ...
                ChartData overallBars = new ChartData()
                {
                    backgroundColor = "red",
                    borderWidth = "1",
                    label = "Загальні рейтингові категорії",
                    data = data
                };
        ...
        private ChartData GetStandardsSummarySingle(CSETContext context, int assessmentId)
        {
          ...
          .ExecuteStoredProc((handler) =>
          {
            ...
            if (results.Count >= 1)
            {
                ...
                myChartData.label = "Підсумок стандартів";
