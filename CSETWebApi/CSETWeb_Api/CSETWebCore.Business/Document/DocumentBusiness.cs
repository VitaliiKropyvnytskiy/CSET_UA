...
namespace CSETWebCore.Business.Document
{
    public class DocumentBusiness : IDocumentBusiness
    {
        ...
        public List<Model.Document.Document> GetDocumentsForAnswer(int answerId)
        {
            ...
            var files = _context.DOCUMENT_ANSWERS.Where(da => da.Answer_Id == answerId).Include(da => da.Document).ToList();
            if (files == null)
            {
                return list;
            }
        ...
        public void RenameDocument(int documentId, int? answerId, string title, int questionId, string questionType, IMaturityBusiness maturityBusiness, IQuestionRequirementManager questionRequirement)
        {
            if (answerId == null)
            {
                var answerObj = _context.ANSWER.FirstOrDefault(x =>
                ...
                if (answerObj == null)
                {
                    ...
                    {
                        answerId = maturityBusiness.StoreAnswer(assessmentId, answer);
                    }
                    else
                    {
                        questionRequirement.InitializeManager(assessmentId);
                        answerId = questionRequirement.StoreAnswer(answer);
                    }
                }
                else
                {
            ...
            var documentAnswer = _context.DOCUMENT_ANSWERS.Where(da => da.Document_Id == documentId && da.Answer_Id == answerId).Include(da => da.Document).FirstOrDefault();
            //var document = _context.DOCUMENT_FILE.Where(d => d.Document_Id == documentId).FirstOrDefault();
            ...
            documentAnswer.Document_Title = title;
            documentAnswer.Document.UpdatedTimestamp = DateTime.Now;

            _context.DOCUMENT_ANSWERS.Update(documentAnswer);
            _context.SaveChanges();
            _assessmentUtil.TouchAssessment(documentAnswer.Document.Assessment_Id);
        }
        ...
        public void AddDocument(string title, int answerId, FileUploadStreamResult result)
        {
            foreach (var file in result.FileResultList)
            {
                ...
                DOCUMENT_ANSWERS temp = new DOCUMENT_ANSWERS()
                {
                    ...
                    Document_Title = string.IsNullOrWhiteSpace(title) ? "натисніть для редагування назви" : title,
                };
        ...
        public List<Model.Document.Document> GetDocumentsForAssessment(int assessmentId)
        {
            ...
            var temp = _context.DOCUMENT_ANSWERS
                .Where(x => answerIds.Contains(x.Answer_Id))
                .Include(x => x.Document)
                .Select(y => new Model.Document.Document
                {
                    Document_Id = y.Document_Id,
                    Title = y.Document_Title == "натисніть для редагування назви" ? "(без назви)" : y.Document_Title,
                    FileName = y.Document.Name
                })
                .ToList();
            list.AddRange(temp);
            return list;
