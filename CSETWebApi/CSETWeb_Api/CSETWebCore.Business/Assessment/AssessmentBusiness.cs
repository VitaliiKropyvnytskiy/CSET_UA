...
namespace CSETWebCore.Business.Assessment
{
    public class AssessmentBusiness : IAssessmentBusiness
    {
        ...
        public AssessmentDetail CreateNewAssessment(int? currentUserId, string workflow, Guid galleryGuid)
        {
            ...
            string defaultExecSumm = "Кібертероризм є реальною та зростаючою загрозою. " +
                "Було розроблено, перевірено та широко прийнято стандарти та посібники, щоб допомогти захиститися від кібератак. " +
                "Інструмент оцінки кібербезпеки(CSET) включає вибірний набір цих стандартів для індивідуальної оцінки кіберуразливості. " +
                "Після того, як стандарти були обрані та отримані набори запитань отримали відповіді, CSET створив підсумок відповідності, " +
                "зібрав статистику відхилень, ранжував найпоширеніші сфери та згенерував рекомендації щодо безпеки.";

            AssessmentDetail newAssessment = new AssessmentDetail
            {
                AssessmentName = (workflow.ToLower() == "acet") ?
                    "ACET 00000 " + DateTime.Now.ToString("MMddyy") : "Нове оцінювання",
        ...
        public AssessmentDetail CreateNewAssessmentForImport(int? currentUserId, string accessKey)
        {
            ...
            AssessmentDetail newAssessment = new AssessmentDetail
            {
                AssessmentName = "Нове оцінювання",
                AssessmentDate = nowUTC,
                CreatorId = currentUserId,
                CreatedDate = nowUTC,
                LastModifiedDate = nowUTC,
        ...
        public int SaveAssessmentDetail(int assessmentId, AssessmentDetail assessment)
        {
            ...
            else
            {
                ...
                _assessmentUtil.SetAssessmentEditStatus(assessmentId, checkInformation(dbInformation));
        ...
        private bool checkInformation(INFORMATION information)
        {
            if (
                (information.Assessment_Name != "Нове оцінювання" && information.Assessment_Name != "(Оцінювання без назви)") ||
                (information.Facility_Name != null && information.Facility_Name != string.Empty) ||
                (information.City_Or_Site_Name != null && information.City_Or_Site_Name != string.Empty) ||
                (information.State_Province_Or_Region != null && information.State_Province_Or_Region != string.Empty)
                )
                return true;
            else
                return false;
